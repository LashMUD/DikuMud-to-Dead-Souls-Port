...
static void create() {
    ...
    SetKeyName("key_3302");
    SetId( ({ "key", "metal key", "item_for_manxam" }) );
    SetAdjectives( ({ "shiny", "metal" }) );
    SetShort("a metal key");
    SetLong("A small metal key with an inscription.");
    SetItems( ([
        ({"inscription"}) : "It is a small insignificant metal key on which the letter \"M\" has been\n"+
                            "inscribed.",
        ]) );        
    SetMass(10);
    SetBaseCost("gold",10);
...
Object name: [key metal], R-number: [91], V-number: [3302] Item type: KEY
