...
static void create() {
    ...
    SetKeyName("key_3321");
    SetId( ({ "key", "iron key", "item_for_mapmaker" }) );
    SetAdjectives( ({ "dull", "iron" }) );
    SetShort("an iron key");
    SetLong("A small iron key with an inscription.");
    SetItems( ([
        ({"inscription"}) : "The small iron key has the letter \"M\" inscribed on its surface.",
        ]) );
    SetMass(10);
    SetBaseCost("gold",10);
}
...
Object name: [key iron], R-number: [110], V-number: [3321] Item type: KEY
